 ...
 * @author sunjiv6
 */
@Api(value = "CustomerOrderListProduct", tags = {"REST APIs to receive Product!!!!","OwnerContent"})
@RestController
@RequestMapping("/customerOrderListProduct")
public class CustomerOrderListController {
    ...
    ProductOrderPlacedService productOrderPlacedService;
    public CustomerOrderListController(OwnerContentService ownerContentService, UserOwnerService userOwnerService,
            UserAdViewerService userAdViewerService,AdViewerLikedItService adViewerLikedItService,
            PricingService pricingService ,ApiKeyService apiKeyService,RetrieveAdsContentService retrieveAdsContentService,
            ProductOrderPlacedService productOrderPlacedService) {
...
//    START TRying
    ...
    @ResponseBody
    public ResponseEntity<ArrayList<AnonymousDto>> ownerSeeHisOwnProductById_myList01(
...
//            OwnerContentDto dtoOwnerContent= null;  
            seeCartInfoCnt=lo.get(0).getSeeCartInfoCnt();
            ArrayList<AnonymousDto> childSanoList=null;
            ArrayList<ProductOrderPlaced> productOrderPlacedList = productOrderPlacedService.getAllOrderForSeller(lo.get(0).getId());
            for(ProductOrderPlaced productOrderPlaced : productOrderPlacedList){                

                childSanoList=new ArrayList<AnonymousDto>();
                OwnerContent modelOwnerContent = this.ownerContentService.getOwnerContentById(productOrderPlaced.getProductId());
                ...
                sanoDto.setP10(modelOwnerContent.getContentName());
                
                
                orderPlacedDto = new AnonymousDto();
                orderPlacedDto.setP1(enc.encryptId(productOrderPlaced.getId()));
                orderPlacedDto.setP2(ContentStatus.deliveryStatusName[Integer.parseInt(productOrderPlaced.getOrderPlacedStatus())]);
//                calendar.setTimeInMillis(Long.parseLong(productOrderPlaced.getOrderStartDate()));
                orderPlacedDto.setP3(productOrderPlaced.getOrderStartDate());//updatedOn
                if(seeCartInfoCnt > 0){
                    orderPlacedDto.setP4(String.valueOf(productOrderPlaced.getQuantity()));
                    orderPlacedDto.setP6(productOrderPlaced.getNewAddress());
                    seeCartInfoCnt--;
                }else{
                    orderPlacedDto.setP4("Your balance is low");
                    orderPlacedDto.setP6("Your balance is low");
                }
                orderPlacedDto.setP11(productOrderPlaced.getUnitDetail());
//                sanoDto.setP13(modelOwnerContent.getPricingId());
                
                childSanoList.add(orderPlacedDto);

                sanoDto.setP16(childSanoList);
                orderplaceAnoDto.add(sanoDto);
            }
            if(seeCartInfoCnt!=lo.get(0).getSeeCartInfoCnt()){
                UserAdViewer updatedUser = lo.get(0);
            
                updatedUser.setSeeCartInfoCnt(seeCartInfoCnt);
                this.userAdViewerService.save(updatedUser);
            }
            response=ResponseEntity.status(HttpStatus.OK).body(orderplaceAnoDto);
        }
        catch(Exception ex){
    ...
    private ArrayList<AnonymousDto> createPricingList(Long id) {
        AnonymousDto panoDto = null;
        ArrayList<AnonymousDto> pricingList= new ArrayList();
        try {
            ArrayList<Pricing> priceList = pricingService.getAllPricingInfoForProductId(id);
            for(Pricing pricing : priceList)
            {
                
                panoDto = new AnonymousDto();
                panoDto.setP1(String.valueOf(pricing.getId()));
                panoDto.setP2(String.valueOf(pricing.getPrice()));
                
                panoDto.setP3(String.valueOf(pricing.getTaxPercent()));
                panoDto.setP4(String.valueOf(pricing.getAvailableQuantity()));
                panoDto.setP6(String.valueOf(pricing.getSoldQuantity()));
                pricingList.add(panoDto);
            }
        } catch (Exception ex) {
            Logger.getLogger(OwnerListProduct.class.getName()).log(Level.SEVERE, null, ex);
        }
        return pricingList;
    }
    
    @PostMapping(value = "/changeDeliveryStatus/{apiUniqueId}/{apiKey}",consumes = {MediaType.MULTIPART_FORM_DATA_VALUE},produces = {MediaType.APPLICATION_JSON_VALUE})
    //@ResponseStatus(value = HttpStatus.CREATED)
    @ResponseBody
    public ResponseEntity<AnonymousDto> changeDeliveryStatus(
            @PathVariable String apiUniqueId,@PathVariable String apiKey,@ModelAttribute AnonymousDto anoDto,BindingResult bindingResult) 
    {
...
//        ArrayList<AnonymousDto> orderplaceAnoDto= new ArrayList();
        ...
        try{        
            if(lo.get(0).getSeeCartInfoCnt()<1){
                sanoDto.setP3("N");
                response = ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                      .body(sanoDto); 
                return response;
            }
            ProductOrderPlaced po = productOrderPlacedService.getProductOrderFromId(enc.decryptId(anoDto.getP3()));
            po.setOrderPlacedStatus(anoDto.getP2());
            productOrderPlacedService.save(po);
            sanoDto.setP3("Y");
            sanoDto.setP2(ContentStatus.deliveryStatusName[Integer.parseInt(anoDto.getP2())]);
...
//                {   securityApi.setOwnerUserId(lo.get(0).getId());}
        ...
        try{        
            ArrayList<OwnerContent> modelOwnerContentList= this.ownerContentService.getAllActiveContentForOwnerUser(
                                                    lo.get(0),ContentStatus.ContentType_Product);
...
//        dtoOwnerContent.setActiveDate_vb(modelOwnerContent.getActiveDate());
        ...
        sanoDto.setP11(modelOwnerContent.getContentName());
        
        Pricing pricing = pricingService.getAcivePricingInfoForProductId(decryptId);
        AnonymousDto panoDto = new AnonymousDto();
        panoDto.setP1(enc.encryptId(pricing.getId()));
        panoDto.setP2(String.valueOf(pricing.getPrice()));
        panoDto.setP3(String.valueOf(pricing.getUnitDetail()));
        panoDto.setP4(String.valueOf(pricing.getAvailableQuantity()));
        panoDto.setP6(String.valueOf(pricing.getTaxPercent()));
        
        panoDto.setP10(String.valueOf(pricing.getTaxDetail()));
