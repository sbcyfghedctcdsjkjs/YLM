...
import 'package:yeslocalmarket01aa/mvvm2/sampleData/CategoryData.dart';
import 'package:yeslocalmarket01aa/mvvm2/utils/constants.dart';
import 'FetchAdsCategory.dart';
import 'dart:ui' as ui;
class FetchAdsCategoryWebService_cl {
  var dio_vb = new Dio();
...
//  }
  ...
  Future<List<FetchAdsCategory_cl>> fectchAdsCategory_mt_data() async {
    print("fectchAdsInPagesByPageNum non temp 1");
    return await CategoryData_cl().getCategoryData();

  }

  Future<List<FetchAdsCategory_cl>> fectchAdsCategory_mt() async {
    print("fectchAdsInPagesByPageNum non temp 1");

    final response_vb = await dio_vb.post(Constants_cl.getAllCategory_mt(),data: {
      "p1": "1",
      "p2": ui.window.locale.languageCode ,
    });
    ...
    if (response_vb.statusCode == 200) {
      ...
      return list.map((result_vb) => FetchAdsCategory_cl.fromJson_mt(result_vb)).toList();
    } else {
      throw Exception("Failled to get ads for the near by region");
    }
  }
